...
%Edges
		\path [->, > = latex, thick] (v1) edge node[midway, sloped, above] {$4$} (v2);
		\path [->, > = latex, thick] (v1) edge node[midway, sloped, below] {$5$} (v3);
		\path [->, > = latex, thick] (v2) edge node[midway, above] {$3$} (v4);
		\path [->, > = latex, thick] (v2) edge node[near end, sloped, above] {$6$} (v5);
		\path [->, > = latex, thick] (v3) edge node[midway, below] {$3$} (v5);
		\path [->, > = latex, thick] (v3) edge node[near end, sloped, above] {$1$} (v4);
		\path [->, > = latex, thick] (v4) edge node[midway, sloped, above] {$2$} (v6);
		\path [->, > = latex, thick] (v5) edge node[midway, sloped, below] {$1$} (v6);
