# -*- coding: utf-8 -*-
"""
Created on Tue Dec  1 12:19:49 2020
...
'''
Vi vil nu lave en repræsentation af grafen. 
'''
def get_graph():
    '''
    Ændrer graph og pre_vertice listerne således de nu er en 11x12 matrix. som
    som kan forstås som en repræsentation af grafen. Hvor mængden -uendelig i graph er
    distancen til punkterne og i pre_vertice er beholdningen. Da vi gerne vil have 
    disse tal til at være så store som muligt for at få den største fortjeneste. Derfor 
    har de værdien minus uendelig nu da det er den dårligste vej.
    '''
    global graph
    global pre_vertice
    for i in range (T):
...
def get_profit():
    '''
    Ændrer matricerne dannet i forrige funktion ud fra graph og pre_vertices 
    således, at graph nu er en matrix over den foreløbige profit i de forskellige punkter. 
    pre_vertices er nu en matrix over lagerbeholdningen i punkterne.
